...
namespace PathTracing
{
    ...
    public class NativeRtxdiPTGenerateInitialSamplesPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiPTGenerateInitialSamplesPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.PtGenerateInitialSamples);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.PtGenerateInitialSamples);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.PtGenerateInitialSamplesCompute);
                ...
                cmd.EndSample(RenderPassMarkers.PtGenerateInitialSamplesCompute);
            }
        }
    }
}
