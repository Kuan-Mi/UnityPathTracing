...
{
    /// <summary>
    /// ReSTIR GI final shading pass. Evaluates the chosen GI reservoir + visibility ray and adds
    /// indirect diffuse/specular contribution to the screen-sized lighting UAVs.
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/GI/FinalShading.hlsl</c>.
    /// numthreads = [RTXDI_SCREEN_SPACE_GROUP_SIZE, RTXDI_SCREEN_SPACE_GROUP_SIZE, 1] (8x8).
    /// </summary>
    public class NativeRtxdiGIFinalShadingPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiGIFinalShadingPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.GI.FinalShading", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.GiFinalShading);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.GiFinalShading);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.GiFinalShadingCompute);
                ...
                cmd.EndSample(RenderPassMarkers.GiFinalShadingCompute);
            }
        }
    }
}
