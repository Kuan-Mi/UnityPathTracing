...
enum GpuTimerPass : uint32_t {
    TIMER_STREAMER = 0,
    TIMER_MORPH_VERTICES,
    TIMER_MORPH_PRIMITIVES,
    TIMER_MORPH_BLAS,
    TIMER_TLAS,
    TIMER_SHARC_UPDATE,
    TIMER_SHARC_RESOLVE,
    TIMER_TRACE_OPAQUE,
    TIMER_SHADOW_DENOISING,
    TIMER_OPAQUE_DENOISING,
    TIMER_COMPOSITION,
    TIMER_TRACE_TRANSPARENT,
    TIMER_REFERENCE,
    TIMER_BEFORE_DLSS,
    TIMER_DLSS,
    TIMER_AFTER_DLSS,
    ...
    TIMER_NIS,
    TIMER_FINAL,
    TIMER_COPY_BACKBUFFER,
    TIMER_PASS_NUM
};

static const char* c_GpuTimerPassNames[TIMER_PASS_NUM] = {
    "Streamer", "Morph vertices", "Morph primitives", "Morph BLAS",
    "TLAS", "SHARC Update", "SHARC Resolve", "Trace opaque",
    "Shadow denoising", "Opaque denoising", "Composition", "Trace transparent",
    "Reference", "Before DLSS", "DLSS", "After DLSS",
    "TAA", "NIS", "Final", "Copy to backbuffer"
};

struct QueuedFrame {
    nri::CommandAllocator* commandAllocator;
    nri::CommandBuffer* commandBuffer;
    nri::Buffer* timestampReadbackBuffer;
};

// RAII GPU timer: writes start timestamp on construction, end timestamp on destruction
struct GpuTimer {
    const nri::CoreInterface& m_NRI;
    nri::CommandBuffer& m_CommandBuffer;
    nri::QueryPool& m_QueryPool;
    uint32_t m_Pass;

    inline GpuTimer(const nri::CoreInterface& NRI, nri::CommandBuffer& commandBuffer, nri::QueryPool& queryPool, uint32_t pass)
        : m_NRI(NRI), m_CommandBuffer(commandBuffer), m_QueryPool(queryPool), m_Pass(pass) {
        m_NRI.CmdEndQuery(m_CommandBuffer, m_QueryPool, m_Pass * 2);
    }
    inline ~GpuTimer() {
        m_NRI.CmdEndQuery(m_CommandBuffer, m_QueryPool, m_Pass * 2 + 1);
    }
};
...
private:
    ...
    nri::QueryPool* m_TimestampQueryPool = nullptr;
    std::array<float, TIMER_PASS_NUM> m_GpuTimings = {};
...
Sample::~Sample() {
    if (NRI.HasCore()) {
        ...
        for (QueuedFrame& queuedFrame : m_QueuedFrames) {
            ...
            NRI.DestroyBuffer(queuedFrame.timestampReadbackBuffer);
        }
        ...
        NRI.DestroyQueryPool(m_TimestampQueryPool);
    }
...
#    endif
    ...
    {
        float3 center = m_Scene.aabb.GetCenter();
        float3 vMin = m_Scene.aabb.vMin;
        printf("aabb.GetCenter() = (%.3f, %.3f, %.3f)\n", center.x, center.y, center.z);
        printf("aabb.vMin        = (%.3f, %.3f, %.3f)\n", vMin.x, vMin.y, vMin.z);
    }
...
void Sample::LatencySleep(uint32_t frameIndex) {
    ...
    // Read GPU timestamps from the completed frame's readback buffer
    if (frameIndex >= GetQueuedFrameNum()) {
        const uint64_t* timestamps = (const uint64_t*)NRI.MapBuffer(*queuedFrame.timestampReadbackBuffer, 0, TIMER_PASS_NUM * 2 * sizeof(uint64_t));
        if (timestamps) {
            const nri::DeviceDesc& deviceDesc = NRI.GetDeviceDesc(*m_Device);
            double invFreq = deviceDesc.other.timestampFrequencyHz ? (1000.0 / (double)deviceDesc.other.timestampFrequencyHz) : 0.0;
            for (uint32_t i = 0; i < TIMER_PASS_NUM; i++) {
                uint64_t t0 = timestamps[i * 2];
                uint64_t t1 = timestamps[i * 2 + 1];
                float ms = (t0 < t1) ? float((t1 - t0) * invFreq) : 0.0f;
                // Smooth the timing with a simple exponential filter
                m_GpuTimings[i] = m_GpuTimings[i] * 0.9f + ms * 0.1f;
            }
            NRI.UnmapBuffer(*queuedFrame.timestampReadbackBuffer);
        }
    }
}
    ...
    if (!IsKeyPressed(Key::LAlt) && m_ShowUi) {
        ...
        ImGui::Begin(buf, nullptr, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoResize);
        {
            ...
            // GPU timings section
            {
                ...
                bool gpuTimingsUnfolded = ImGui::CollapsingHeader("GPU TIMINGS (ms)", ImGuiTreeNodeFlags_CollapsingHeader);
                ImGui::PopStyleColor();
                ImGui::PopStyleColor();

                if (gpuTimingsUnfolded) {
                    float totalGpuMs = 0.0f;
                    for (uint32_t i = 0; i < TIMER_PASS_NUM; i++) {
                        if (m_GpuTimings[i] > 0.0f) {
                            ImGui::Text("%-24s %.3f ms", c_GpuTimerPassNames[i], m_GpuTimings[i]);
                            totalGpuMs += m_GpuTimings[i];
                        }
                    }
                    ImGui::Separator();
                    ImGui::Text("%-24s %.3f ms", "Total", totalGpuMs);
                }
            }
...
// Use this code to convert tests to reflect new Settings and Camera layouts
        ...
        // GlobalConstants inspector window
        {
            const GlobalConstants& gc = m_GlobalConstants;
            if (ImGui::CollapsingHeader("Matrices")) {
                auto showMat = [](const char* name, const float4x4& m) {
                    ImGui::Text("%s:", name);
                    ImGui::Text("  [%.3f %.3f %.3f %.3f]", m.ca[0].x, m.ca[0].y, m.ca[0].z, m.ca[0].w);
                    ImGui::Text("  [%.3f %.3f %.3f %.3f]", m.ca[1].x, m.ca[1].y, m.ca[1].z, m.ca[1].w);
                    ImGui::Text("  [%.3f %.3f %.3f %.3f]", m.ca[2].x, m.ca[2].y, m.ca[2].z, m.ca[2].w);
                    ImGui::Text("  [%.3f %.3f %.3f %.3f]", m.ca[3].x, m.ca[3].y, m.ca[3].z, m.ca[3].w);
                };
                showMat("gViewToWorld",      gc.gViewToWorld);
                showMat("gViewToClip",       gc.gViewToClip);
                showMat("gWorldToView",      gc.gWorldToView);
                showMat("gWorldToClip",      gc.gWorldToClip);
                showMat("gWorldToViewPrev",  gc.gWorldToViewPrev);
                showMat("gWorldToClipPrev",  gc.gWorldToClipPrev);
                showMat("gViewToWorldPrev",  gc.gViewToWorldPrev);
            }
            if (ImGui::CollapsingHeader("float4 vectors")) {
                ImGui::Text("gHitDistSettings:       %.3f %.3f %.3f %.3f", gc.gHitDistSettings.x,     gc.gHitDistSettings.y,     gc.gHitDistSettings.z,     gc.gHitDistSettings.w);
                ImGui::Text("gCameraFrustum:         %.3f %.3f %.3f %.3f", gc.gCameraFrustum.x,       gc.gCameraFrustum.y,       gc.gCameraFrustum.z,       gc.gCameraFrustum.w);
                ImGui::Text("gSunBasisX:             %.3f %.3f %.3f %.3f", gc.gSunBasisX.x,           gc.gSunBasisX.y,           gc.gSunBasisX.z,           gc.gSunBasisX.w);
                ImGui::Text("gSunBasisY:             %.3f %.3f %.3f %.3f", gc.gSunBasisY.x,           gc.gSunBasisY.y,           gc.gSunBasisY.z,           gc.gSunBasisY.w);
                ImGui::Text("gSunDirection:          %.3f %.3f %.3f %.3f", gc.gSunDirection.x,        gc.gSunDirection.y,        gc.gSunDirection.z,        gc.gSunDirection.w);
                ImGui::Text("gCameraGlobalPos:       %.3f %.3f %.3f %.3f", gc.gCameraGlobalPos.x,     gc.gCameraGlobalPos.y,     gc.gCameraGlobalPos.z,     gc.gCameraGlobalPos.w);
                ImGui::Text("gCameraGlobalPosPrev:   %.3f %.3f %.3f %.3f", gc.gCameraGlobalPosPrev.x, gc.gCameraGlobalPosPrev.y, gc.gCameraGlobalPosPrev.z, gc.gCameraGlobalPosPrev.w);
                ImGui::Text("gViewDirection:         %.3f %.3f %.3f %.3f", gc.gViewDirection.x,       gc.gViewDirection.y,       gc.gViewDirection.z,       gc.gViewDirection.w);
                ImGui::Text("gHairBaseColor:         %.3f %.3f %.3f %.3f", gc.gHairBaseColor.x,       gc.gHairBaseColor.y,       gc.gHairBaseColor.z,       gc.gHairBaseColor.w);
            }
            if (ImGui::CollapsingHeader("float2 vectors")) {
                ImGui::Text("gHairBetas:              %.3f %.3f", gc.gHairBetas.x,          gc.gHairBetas.y);
                ImGui::Text("gOutputSize:             %.1f %.1f", gc.gOutputSize.x,         gc.gOutputSize.y);
                ImGui::Text("gRenderSize:             %.1f %.1f", gc.gRenderSize.x,         gc.gRenderSize.y);
                ImGui::Text("gRectSize:               %.1f %.1f", gc.gRectSize.x,           gc.gRectSize.y);
                ImGui::Text("gInvOutputSize:          %.6f %.6f", gc.gInvOutputSize.x,      gc.gInvOutputSize.y);
                ImGui::Text("gInvRenderSize:          %.6f %.6f", gc.gInvRenderSize.x,      gc.gInvRenderSize.y);
                ImGui::Text("gInvRectSize:            %.6f %.6f", gc.gInvRectSize.x,        gc.gInvRectSize.y);
                ImGui::Text("gRectSizePrev:           %.1f %.1f", gc.gRectSizePrev.x,       gc.gRectSizePrev.y);
                ImGui::Text("gInvSharcRenderSize:     %.6f %.6f", gc.gInvSharcRenderSize.x, gc.gInvSharcRenderSize.y);
                ImGui::Text("gJitter:                 %.6f %.6f", gc.gJitter.x,             gc.gJitter.y);
                ImGui::Text("gJitterPrev:             %.6f %.6f", gc.gJitterPrev.x,         gc.gJitterPrev.y);
            }
            if (ImGui::CollapsingHeader("float scalars")) {
                ImGui::Text("gEmissionIntensityLights:        %.4f", gc.gEmissionIntensityLights);
                ImGui::Text("gEmissionIntensityCubes:         %.4f", gc.gEmissionIntensityCubes);
                ImGui::Text("gNearZ:                          %.6f", gc.gNearZ);
                ImGui::Text("gSeparator:                      %.3f", gc.gSeparator);
                ImGui::Text("gRoughnessOverride:              %.3f", gc.gRoughnessOverride);
                ImGui::Text("gMetalnessOverride:              %.3f", gc.gMetalnessOverride);
                ImGui::Text("gUnitToMetersMultiplier:         %.6f", gc.gUnitToMetersMultiplier);
                ImGui::Text("gTanSunAngularRadius:            %.6f", gc.gTanSunAngularRadius);
                ImGui::Text("gTanPixelAngularRadius:          %.6f", gc.gTanPixelAngularRadius);
                ImGui::Text("gDebug:                          %.4f", gc.gDebug);
                ImGui::Text("gPrevFrameConfidence:            %.4f", gc.gPrevFrameConfidence);
                ImGui::Text("gUnproject:                      %.6f", gc.gUnproject);
                ImGui::Text("gAperture:                       %.6f", gc.gAperture);
                ImGui::Text("gFocalDistance:                  %.4f", gc.gFocalDistance);
                ImGui::Text("gFocalLength:                    %.6f", gc.gFocalLength);
                ImGui::Text("gTAA:                            %.6f", gc.gTAA);
                ImGui::Text("gHdrScale:                       %.4f", gc.gHdrScale);
                ImGui::Text("gExposure:                       %.4f", gc.gExposure);
                ImGui::Text("gMipBias:                        %.1f", gc.gMipBias);
                ImGui::Text("gOrthoMode:                      %.1f", gc.gOrthoMode);
                ImGui::Text("gMinProbability:                 %.6f", gc.gMinProbability);
            }
            if (ImGui::CollapsingHeader("uint32 scalars")) {
                ImGui::Text("gMaxAccumulatedFrameNum:                  %u", gc.gMaxAccumulatedFrameNum);
                ImGui::Text("gDenoiserType:                           %u", gc.gDenoiserType);
                ImGui::Text("gDisableShadowsAndEnableImportanceSampling: %u", gc.gDisableShadowsAndEnableImportanceSampling);
                ImGui::Text("gFrameIndex:                             %u", gc.gFrameIndex);
                ImGui::Text("gForcedMaterial:                         %u", gc.gForcedMaterial);
                ImGui::Text("gUseNormalMap:                           %u", gc.gUseNormalMap);
                ImGui::Text("gBounceNum:                              %u", gc.gBounceNum);
                ImGui::Text("gResolve:                                %u", gc.gResolve);
                ImGui::Text("gValidation:                             %u", gc.gValidation);
                ImGui::Text("gSR:                                     %u", gc.gSR);
                ImGui::Text("gRR:                                     %u", gc.gRR);
                ImGui::Text("gIsSrgb:                                 %u", gc.gIsSrgb);
                ImGui::Text("gOnScreen:                               %u", gc.gOnScreen);
                ImGui::Text("gTracingMode:                            %u", gc.gTracingMode);
                ImGui::Text("gSampleNum:                              %u", gc.gSampleNum);
                ImGui::Text("gPSR:                                    %u", gc.gPSR);
                ImGui::Text("gSHARC:                                  %u", gc.gSHARC);
                ImGui::Text("gTrimLobe:                               %u", gc.gTrimLobe);
            }
        }
        // NRD CommonSettings inspector
        {
            const nrd::CommonSettings& cs = m_CommonSettings;
            if (ImGui::CollapsingHeader("NRD CommonSettings")) {
                ImGui::Text("motionVectorScale:              %.6f %.6f %.6f", cs.motionVectorScale[0], cs.motionVectorScale[1], cs.motionVectorScale[2]);
                ImGui::Text("cameraJitter:                  %.6f %.6f", cs.cameraJitter[0], cs.cameraJitter[1]);
                ImGui::Text("cameraJitterPrev:              %.6f %.6f", cs.cameraJitterPrev[0], cs.cameraJitterPrev[1]);
                ImGui::Text("resourceSize:                  %u %u", (uint32_t)cs.resourceSize[0], (uint32_t)cs.resourceSize[1]);
                ImGui::Text("resourceSizePrev:              %u %u", (uint32_t)cs.resourceSizePrev[0], (uint32_t)cs.resourceSizePrev[1]);
                ImGui::Text("rectSize:                      %u %u", (uint32_t)cs.rectSize[0], (uint32_t)cs.rectSize[1]);
                ImGui::Text("rectSizePrev:                  %u %u", (uint32_t)cs.rectSizePrev[0], (uint32_t)cs.rectSizePrev[1]);
                ImGui::Text("viewZScale:                    %.6f", cs.viewZScale);
                ImGui::Text("timeDeltaBetweenFrames:        %.6f", cs.timeDeltaBetweenFrames);
                ImGui::Text("denoisingRange:                %.3f", cs.denoisingRange);
                ImGui::Text("disocclusionThreshold:         %.4f", cs.disocclusionThreshold);
                ImGui::Text("disocclusionThresholdAlternate:%.4f", cs.disocclusionThresholdAlternate);
                ImGui::Text("cameraAttachedReflectionMaterialID: %.1f", cs.cameraAttachedReflectionMaterialID);
                ImGui::Text("strandMaterialID:              %.1f", cs.strandMaterialID);
                ImGui::Text("strandThickness:               %.8f", cs.strandThickness);
                ImGui::Text("historyFixAlternatePixelStrideMaterialID: %.1f", cs.historyFixAlternatePixelStrideMaterialID);
                ImGui::Text("splitScreen:                   %.3f", cs.splitScreen);
                ImGui::Text("debug:                         %.4f", cs.debug);
                ImGui::Text("rectOrigin:                    %u %u", cs.rectOrigin[0], cs.rectOrigin[1]);
                ImGui::Text("frameIndex:                    %u", cs.frameIndex);
                ImGui::Text("accumulationMode:              %d", (int)cs.accumulationMode);
                ImGui::Text("isMotionVectorInWorldSpace:    %s", cs.isMotionVectorInWorldSpace ? "true" : "false");
                ImGui::Text("isHistoryConfidenceAvailable:  %s", cs.isHistoryConfidenceAvailable ? "true" : "false");
                ImGui::Text("isDisocclusionThresholdMixAvailable: %s", cs.isDisocclusionThresholdMixAvailable ? "true" : "false");
                ImGui::Text("enableValidation:              %s", cs.enableValidation ? "true" : "false");
            }
        }
        // NRD SigmaSettings inspector
        {
            const nrd::SigmaSettings& ss = m_SigmaSettings;
            if (ImGui::CollapsingHeader("NRD SigmaSettings")) {
                ImGui::Text("lightDirection:                %.4f %.4f %.4f", ss.lightDirection[0], ss.lightDirection[1], ss.lightDirection[2]);
                ImGui::Text("planeDistanceSensitivity:      %.4f", ss.planeDistanceSensitivity);
                ImGui::Text("maxStabilizedFrameNum:         %u", ss.maxStabilizedFrameNum);
            }
        }
        // NRD RelaxSettings inspector
        {
            const nrd::RelaxSettings& rs = m_RelaxSettings;
            if (ImGui::CollapsingHeader("NRD RelaxSettings")) {
                ImGui::Text("diffuseMaxAccumulatedFrameNum: %u", rs.diffuseMaxAccumulatedFrameNum);
                ImGui::Text("specularMaxAccumulatedFrameNum:%u", rs.specularMaxAccumulatedFrameNum);
                ImGui::Text("diffuseMaxFastAccumulatedFrameNum: %u", rs.diffuseMaxFastAccumulatedFrameNum);
                ImGui::Text("specularMaxFastAccumulatedFrameNum:%u", rs.specularMaxFastAccumulatedFrameNum);
                ImGui::Text("historyFixFrameNum:            %u", rs.historyFixFrameNum);
                ImGui::Text("historyFixBasePixelStride:     %u", rs.historyFixBasePixelStride);
                ImGui::Text("historyFixAlternatePixelStride:%u", rs.historyFixAlternatePixelStride);
                ImGui::Text("historyFixEdgeStoppingNormalPower: %.3f", rs.historyFixEdgeStoppingNormalPower);
                ImGui::Text("fastHistoryClampingSigmaScale: %.3f", rs.fastHistoryClampingSigmaScale);
                ImGui::Text("diffusePrepassBlurRadius:      %.3f", rs.diffusePrepassBlurRadius);
                ImGui::Text("specularPrepassBlurRadius:     %.3f", rs.specularPrepassBlurRadius);
                ImGui::Text("minHitDistanceWeight:          %.4f", rs.minHitDistanceWeight);
                ImGui::Text("spatialVarianceEstimationHistoryThreshold: %u", rs.spatialVarianceEstimationHistoryThreshold);
                ImGui::Text("diffusePhiLuminance:           %.3f", rs.diffusePhiLuminance);
                ImGui::Text("specularPhiLuminance:          %.3f", rs.specularPhiLuminance);
                ImGui::Text("lobeAngleFraction:             %.4f", rs.lobeAngleFraction);
                ImGui::Text("roughnessFraction:             %.4f", rs.roughnessFraction);
                ImGui::Text("specularVarianceBoost:         %.3f", rs.specularVarianceBoost);
                ImGui::Text("specularLobeAngleSlack:        %.4f", rs.specularLobeAngleSlack);
                ImGui::Text("atrousIterationNum:            %u", rs.atrousIterationNum);
                ImGui::Text("diffuseMinLuminanceWeight:     %.4f", rs.diffuseMinLuminanceWeight);
                ImGui::Text("specularMinLuminanceWeight:    %.4f", rs.specularMinLuminanceWeight);
                ImGui::Text("depthThreshold:                %.5f", rs.depthThreshold);
                ImGui::Text("confidenceDrivenRelaxationMultiplier: %.3f", rs.confidenceDrivenRelaxationMultiplier);
                ImGui::Text("confidenceDrivenLuminanceEdgeStoppingRelaxation: %.3f", rs.confidenceDrivenLuminanceEdgeStoppingRelaxation);
                ImGui::Text("confidenceDrivenNormalEdgeStoppingRelaxation: %.3f", rs.confidenceDrivenNormalEdgeStoppingRelaxation);
                ImGui::Text("luminanceEdgeStoppingRelaxation: %.3f", rs.luminanceEdgeStoppingRelaxation);
                ImGui::Text("normalEdgeStoppingRelaxation:  %.3f", rs.normalEdgeStoppingRelaxation);
                ImGui::Text("roughnessEdgeStoppingRelaxation: %.3f", rs.roughnessEdgeStoppingRelaxation);
                ImGui::Text("checkerboardMode:              %d", (int)rs.checkerboardMode);
                ImGui::Text("hitDistanceReconstructionMode: %d", (int)rs.hitDistanceReconstructionMode);
                ImGui::Text("minMaterialForDiffuse:         %.1f", rs.minMaterialForDiffuse);
                ImGui::Text("minMaterialForSpecular:        %.1f", rs.minMaterialForSpecular);
                ImGui::Text("enableAntiFirefly:             %s", rs.enableAntiFirefly ? "true" : "false");
                ImGui::Text("enableRoughnessEdgeStopping:   %s", rs.enableRoughnessEdgeStopping ? "true" : "false");
                ImGui::Separator();
                ImGui::Text("Antilag:");
                ImGui::Text("  accelerationAmount:          %.3f", rs.antilagSettings.accelerationAmount);
                ImGui::Text("  spatialSigmaScale:           %.3f", rs.antilagSettings.spatialSigmaScale);
                ImGui::Text("  temporalSigmaScale:          %.3f", rs.antilagSettings.temporalSigmaScale);
                ImGui::Text("  resetAmount:                 %.3f", rs.antilagSettings.resetAmount);
            }
        }
        ImGui::End();
    }
    ...
    if(0)
    // Draw scene AABB wireframe overlay (uses previous frame camera, one frame lag is acceptable)
    {
        const float3& bMin = m_Scene.aabb.vMin;
        const float3& bMax = m_Scene.aabb.vMax;
        ...
        const float2 outputSize = float2((float)GetOutputResolution().x, (float)GetOutputResolution().y);
        const float4x4& worldToClip = m_Camera.state.mWorldToClip;
        const float3 camPos = float3(m_Camera.state.globalPosition);

        auto Project = [&](const float3& p, ImVec2& out) -> bool {
            float4 clip = worldToClip * float4(p - camPos, 1.0f);
            if (clip.w <= 0.0f)
                return false;
            ...
            if (nx < -1.5f || nx > 1.5f || ny < -1.5f || ny > 1.5f)
                return false;
            out.x = ( nx * 0.5f + 0.5f) * outputSize.x;
            out.y = (-ny * 0.5f + 0.5f) * outputSize.y;
            return true;
        };

        // Print first-frame debug info once
        static bool debugPrinted = false;
        if (!debugPrinted) {
            debugPrinted = true;
            printf("AABB vMin=(%.2f,%.2f,%.2f) vMax=(%.2f,%.2f,%.2f)\n",
                bMin.x, bMin.y, bMin.z, bMax.x, bMax.y, bMax.z);
            printf("CamPos=(%.2f,%.2f,%.2f)\n", camPos.x, camPos.y, camPos.z);
            for (int i = 0; i < 8; i++) {
                float4 clip = worldToClip * float4(corners[i] - camPos, 1.0f);
                printf("  corner[%d] clip=(%.3f,%.3f,%.3f,%.3f)\n", i, clip.x, clip.y, clip.z, clip.w);
            }
        }
        ...
        bool visible[8];
        for (int i = 0; i < 8; i++)
            visible[i] = Project(corners[i], screenCorners[i]);

        ImDrawList* dl = ImGui::GetBackgroundDrawList();
        const ImU32 color = IM_COL32(255, 255, 0, 255);
        for (const auto& e : edges) {
            if (visible[e[0]] && visible[e[1]])
                dl->AddLine(screenCorners[e[0]], screenCorners[e[1]], color, 2.0f);
        }

        // Draw line from camera start position (aabb center) to look-at target (aabb vMin)
        ImVec2 lineStart, lineEnd;
        bool startVis = Project(m_Scene.aabb.GetCenter(), lineStart);
        bool endVis   = Project(m_Scene.aabb.vMin, lineEnd);
        if (startVis && endVis)
            dl->AddLine(lineStart, lineEnd, IM_COL32(255, 80, 0, 255), 2.0f);
        if (startVis)
            dl->AddCircleFilled(lineStart, 5.0f, IM_COL32(255, 80, 0, 255));
        if (endVis)
            dl->AddCircleFilled(lineEnd, 5.0f, IM_COL32(0, 200, 255, 255));
    }
...
void Sample::CreateCommandBuffers() {
    // Timestamp query pool
    {
        ...
        queryPoolDesc.capacity = TIMER_PASS_NUM * 2;
        NRI_ABORT_ON_FAILURE(NRI.CreateQueryPool(*m_Device, queryPoolDesc, m_TimestampQueryPool));
    }
    ...
    for (QueuedFrame& queuedFrame : m_QueuedFrames) {
        ...
        // Per-frame readback buffer for timestamp results
        nri::BufferDesc bufferDesc = {TIMER_PASS_NUM * 2 * sizeof(uint64_t), 0, nri::BufferUsageBits::NONE};
        NRI_ABORT_ON_FAILURE(NRI.CreateCommittedBuffer(*m_Device, nri::MemoryLocation::HOST_READBACK, 0.0f, bufferDesc, queuedFrame.timestampReadbackBuffer));
    }
}
...
void Sample::CreateAccelerationStructures() {
    ...
    for (uint32_t i = m_ProxyInstancesNum; i < m_Scene.instances.size(); i++) {
        ...
        if (material.IsOff()){
            printf("Warning: instance %s has OFF material, it will be skipped\n", instance.name.c_str());
            continue;
        }
        ...
        if (instance.allowUpdate) {
            ...
            if (!material.IsTransparent()) {
                ...
                printf("instance %s has transparent material\n", instance.name.c_str());
            }
    ...
    // Compact BLASes
    std::vector<uint64_t> compactedBlasSizes;
    std::vector<nri::AccelerationStructure*> compactedBlases;
    {
        ...
        compactedBlasSizes.assign(sizes, sizes + blasNum);
    ...
    // Collect pre-compaction sizes before destroying temp BLASes
    std::vector<uint64_t> preCompactionBlasSizes(blasNum);
    for (uint32_t i = 0; i < blasNum; i++) {
        nri::MemoryDesc memoryDesc = {};
        NRI.GetAccelerationStructureMemoryDesc(*buildBottomLevelAccelerationStructureDescs[i].dst, nri::MemoryLocation::DEVICE, memoryDesc);
        preCompactionBlasSizes[i] = memoryDesc.size;
    }
    ...
    printf(
        ...
        "  Scratch size  : %.2f Mb\n"
        "  BLAS num      : %u\n"
        "  m_OpaqueObjectsNum : %u\n"
        "  m_TransparentObjectsNum : %u\n"
        "  m_EmissiveObjectsNum : %u\n"
        "  Geometries    : %zu\n"
        "  Primitives    : %zu\n",
        ...
        blasNum, m_OpaqueObjectsNum, m_TransparentObjectsNum, m_EmissiveObjectsNum,
        geometries.size(), primitivesNum);

    printf("  BLAS sizes (compacted):\n");
    for (uint32_t i = 0; i < blasNum; i++)
        printf("    [%u] %.3f Mb -> %.3f Mb\n", i, preCompactionBlasSizes[i] / (1024.0 * 1024.0), compactedBlasSizes[i] / (1024.0 * 1024.0));
}
...
void Sample::RenderFrame(uint32_t frameIndex) {
    ...
    // Reset timestamp query pool for this frame
    NRI.CmdResetQueries(commandBuffer, *m_TimestampQueryPool, 0, TIMER_PASS_NUM * 2);
    ...
    { // Copy upload requests to destinations
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_STREAMER);
    ...
    if (m_Settings.activeAnimation < m_Scene.animations.size() && m_Scene.animations[m_Settings.activeAnimation].morphMeshInstances.size() && (!m_Settings.pauseAnimation || !m_SettingsPrev.pauseAnimation || frameIndex == 0)) {
        ...
        { // Update vertices
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_MORPH_VERTICES);
        ...
        { // Update primitives
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_MORPH_PRIMITIVES);
        ...
        { // Update BLAS
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_MORPH_BLAS);
    ...
    { // TLAS and SHARC clear
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_TLAS);
    ...
    { // SHARC
        ...
        { // Update
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_SHARC_UPDATE);
        ...
        { // Resolve
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_SHARC_RESOLVE);
    ...
    { // Trace opaque
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_TRACE_OPAQUE);
    ...
    { // Shadow denoising
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_SHADOW_DENOISING);
    ...
    { // Opaque denoising
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_OPAQUE_DENOISING);
...
#    endif
    ...
    { // Composition
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_COMPOSITION);
    ...
    { // Trace transparent
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_TRACE_TRANSPARENT);
    ...
    if (m_Settings.denoiser == DENOISER_REFERENCE) { // Reference
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_REFERENCE);
    ...
    if (IsDlssEnabled()) {
        ...
        if (m_Settings.SR) {
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_BEFORE_DLSS);
        ...
        { // DLSS
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_DLSS);
        ...
        { // After DLSS
            ...
            GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_AFTER_DLSS);
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_TAA);
    ...
    { // NIS
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_NIS);
    ...
    { // Final
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_FINAL);
    ...
    { // Copy to back-buffer
        ...
        GpuTimer gpuTimer(NRI, commandBuffer, *m_TimestampQueryPool, TIMER_COPY_BACKBUFFER);
    ...
    // RECORDING END
    // Copy timestamp query results to readback buffer for CPU readback next frame
    NRI.CmdCopyQueries(commandBuffer, *m_TimestampQueryPool, 0, TIMER_PASS_NUM * 2, *queuedFrame.timestampReadbackBuffer, 0);
    NRI.EndCommandBuffer(commandBuffer);
