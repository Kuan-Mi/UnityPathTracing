...
{
    /// <summary>
    /// Shades the secondary surfaces emitted by <see cref="NativeRtxdiBrdfRayTracingPass"/> using
    /// ReSTIR DI on the secondary hit (so indirect lighting itself benefits from RIS sampling),
    /// then writes initial GI reservoirs.
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/ShadeSecondarySurfaces.hlsl</c>.
    /// numthreads = [RTXDI_SCREEN_SPACE_GROUP_SIZE, RTXDI_SCREEN_SPACE_GROUP_SIZE, 1] (8x8).
    /// </summary>
    public class NativeRtxdiShadeSecondarySurfacesPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiShadeSecondarySurfacesPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.ShadeSecondarySurfaces", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.ShadeSecondarySurfaces);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.ShadeSecondarySurfaces);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.ShadeSecondarySurfacesCompute);
                ...
                cmd.EndSample(RenderPassMarkers.ShadeSecondarySurfacesCompute);
            }
        }
    }
}
