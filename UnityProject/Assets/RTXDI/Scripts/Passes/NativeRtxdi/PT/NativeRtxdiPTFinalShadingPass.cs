...
{
    /// <summary>
    /// ReSTIR PT final shading pass. Evaluates the chosen PT reservoir and accumulates
    /// path-traced radiance into the screen-sized lighting UAVs.
    /// Always the last pass in the PT pipeline.
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/PT/FinalShading.hlsl</c>.
    /// numthreads = [RTXDI_SCREEN_SPACE_GROUP_SIZE, RTXDI_SCREEN_SPACE_GROUP_SIZE, 1] (8x8).
    /// </summary>
    public class NativeRtxdiPTFinalShadingPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiPTFinalShadingPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.PT.FinalShading", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.PtFinalShading);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.PtFinalShading);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.PtFinalShadingCompute);
                ...
                cmd.EndSample(RenderPassMarkers.PtFinalShadingCompute);
            }
        }
    }
}
