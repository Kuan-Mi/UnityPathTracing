...
#include "Assets/Shaders/Include/NativeRayTracingShared.hlsl"

#include "Assets/Shaders/NRD/NRD.hlsli"
#pragma max_recursion_depth 1

// Input
StructuredBuffer<uint4> gIn_ScramblingRanking: register(t0, space1);
StructuredBuffer<uint4> gIn_Sobol: register(t1, space1);
Texture2D<float3> gIn_PrevComposedDiff: register(t2, space1);
Texture2D<float4> gIn_PrevComposedSpec_PrevViewZ: register(t3, space1);

// Output
RWTexture2D<float3> g_Output: register(u0, space1);

// 运动矢量（Motion Vector），用于描述像素在当前帧与上一帧之间的运动，以及视深（ViewZ）和TAA遮罩信息。
RWTexture2D<float4> gOut_Mv: register(u1, space1);
// 视空间深度（ViewZ），即像素在视空间中的Z值。
RWTexture2D<float> gOut_ViewZ: register(u2, space1);
// 法线、粗糙度和材质ID的打包信息。用于后续的去噪和材质区分。
RWTexture2D<float4> gOut_Normal_Roughness: register(u3, space1);
// 基础色（BaseColor，已转为sRGB）和金属度（Metalness）。
RWTexture2D<float4> gOut_BaseColor_Metalness: register(u4, space1);

RWTexture2D<uint> gOut_GeoNormal: register(u5, space1);

// 路径追踪累积通量（Path Traced Throughput），用于存储路径追踪过程中光线的累积贡献。
RWTexture2D<float3> gOut_PsrThroughput: register(u6, space1);

// 直接光照（Direct Lighting），即主光线命中点的直接光照结果。
RWTexture2D<float3> gOut_DirectLighting: register(u7, space1);
// 直接自发光（Direct Emission），即材质的自发光分量。
RWTexture2D<float3> gOut_DirectEmission: register(u8, space1);

// 阴影数据（Shadow Data），如半影宽度等，用于软阴影和去噪。
RWTexture2D<float> gOut_ShadowData: register(u9, space1);
// 漫反射光照结果（Diffuse Radiance），包含噪声和打包后的信息。
RWTexture2D<float4> gOut_Diff: register(u10, space1);
// 高光反射光照结果（Specular Radiance），包含噪声和打包后的信息。
RWTexture2D<float4> gOut_Spec: register(u11, space1);
