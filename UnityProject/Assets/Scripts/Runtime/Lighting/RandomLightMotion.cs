...
// 由 RandomLightSpawner.GenerateLights() 自动挂载并初始化。
    ...
    public class RandomLightMotion : MonoBehaviour
    {
        ...
        [HideInInspector]
        public Transform spawnerTransform; // 生成器 Transform（包围盒中心）

        [HideInInspector]
        public Vector3 boundsSize; // 包围盒尺寸（本地空间）

        [HideInInspector]
        public float moveSpeed = 1f; // 移动速度（世界单位/秒）

        [HideInInspector]
        public float rotSpeed = 30f; // 朝向变化速度（度/秒），仅 Spot/Directional
