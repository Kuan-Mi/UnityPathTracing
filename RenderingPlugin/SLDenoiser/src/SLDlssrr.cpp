...
namespace SLDlssrr
{
    ...
    void Dispatch(SLDlssrrFrameData* data, ID3D12GraphicsCommandList* cmdList)
    {
        ...
        // so evaluate tags against the same token as DLSS-G. The main-thread frame tick mints it
        // every frame in both the editor and the player (see SLStreamlineFrameLoop), so it is
        // non-null whenever the evaluate runs.
