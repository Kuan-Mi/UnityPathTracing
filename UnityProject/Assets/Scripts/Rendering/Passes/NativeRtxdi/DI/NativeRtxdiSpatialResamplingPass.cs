...
namespace PathTracing
{
    ...
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/DI/SpatialResampling.hlsl</c>
    /// (compiled from <c>SpatialResampling.computeshader</c>).
    ...
    /// </summary>
    public class NativeRtxdiSpatialResamplingPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiSpatialResamplingPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.DI.SpatialResampling", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.DiSpatialResampling);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.DiSpatialResampling);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.DiSpatialResamplingCompute);
                ...
                cmd.EndSample(RenderPassMarkers.DiSpatialResamplingCompute);
            }
        }
    }
}
