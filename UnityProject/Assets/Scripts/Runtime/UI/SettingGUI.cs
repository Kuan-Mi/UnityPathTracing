...
namespace PathTracing
{
    public abstract class SettingGUI<TFeature, TSettings> : MonoBehaviour where TFeature : ScriptableRendererFeature
    {
        ...
        private void OnGUI()
        {
            ...
            if (_feature == null)
            {
                GUI.Box(new Rect(Screen.width - 360, (int)btnH + 20, 350, 28), "SettingGUI: Feature not found");
                return;
            }

            _windowRect = GUI.Window(0xBEEF, _windowRect, DrawWindow, $"{GetSettingName()} Settings  [" + toggleKey + "]");
        }
        ...
        protected void DrawWindow(int id)
        {
            ...
            // set mian light dir form GUI using Azimuth and Elevation
            if (mainLight != null)
            {
                GUILayout.Label("Main Light Direction", _boldLabel, GUILayout.Width(215));
                azimuth = GUILayout.HorizontalSlider(azimuth, 0, 360, GUILayout.Width(145));
                GUILayout.Label(azimuth.ToString("F1") + "°", _label, GUILayout.Width(40));
                elevation = GUILayout.HorizontalSlider(elevation, -90, 90, GUILayout.Width(145));
                GUILayout.Label(elevation.ToString("F1") + "°", _label, GUILayout.Width(40));

                // Convert azimuth and elevation to a direction vector
                float   azRad = math.radians(azimuth);
                float   elRad = math.radians(elevation);
                Vector3 dir   = new Vector3(math.cos(elRad) * math.sin(azRad), math.sin(elRad), math.cos(elRad) * math.cos(azRad));
                mainLight.rotation = Quaternion.LookRotation(dir);
            }
        ...
        private static TFeature FindFeature()
        {
            var cam = Camera.main;
