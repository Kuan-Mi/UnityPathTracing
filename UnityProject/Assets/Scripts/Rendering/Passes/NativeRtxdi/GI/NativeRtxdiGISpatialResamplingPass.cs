...
namespace PathTracing
{
    ...
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/GI/SpatialResampling.hlsl</c>.
    /// numthreads = [RTXDI_SCREEN_SPACE_GROUP_SIZE, RTXDI_SCREEN_SPACE_GROUP_SIZE, 1] (8x8).
    /// </summary>
    public class NativeRtxdiGISpatialResamplingPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiGISpatialResamplingPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.GI.SpatialResampling", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.GiSpatialResampling);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.GiSpatialResampling);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.GiSpatialResamplingCompute);
                ...
                cmd.EndSample(RenderPassMarkers.GiSpatialResamplingCompute);
            }
        }
    }
}
