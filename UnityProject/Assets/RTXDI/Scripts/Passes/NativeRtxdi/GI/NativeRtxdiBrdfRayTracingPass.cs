...
{
    /// <summary>
    /// Indirect lighting initial-bounce pass. Inline RayQuery against <c>SceneBVH</c>
    /// emits <c>SecondaryGBuffer</c> + initial GI reservoir samples.
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/BrdfRayTracing.hlsl</c>.
    /// numthreads = [RTXDI_SCREEN_SPACE_GROUP_SIZE, RTXDI_SCREEN_SPACE_GROUP_SIZE, 1] (8x8).
    /// </summary>
    public class NativeRtxdiBrdfRayTracingPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiBrdfRayTracingPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.BrdfRayTracing", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.BrdfRayTracing);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.BrdfRayTracing);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.BrdfRayTracingCompute);
                ...
                cmd.EndSample(RenderPassMarkers.BrdfRayTracingCompute);
            }
        }
    }
}
