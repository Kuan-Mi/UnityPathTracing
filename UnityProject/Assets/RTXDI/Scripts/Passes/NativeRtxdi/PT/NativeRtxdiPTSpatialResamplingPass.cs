...
{
    /// <summary>
    /// ReSTIR PT spatial resampling pass. Resamples PT reservoirs from spatial neighbours.
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/PT/SpatialResampling.hlsl</c>.
    /// numthreads = [RTXDI_SCREEN_SPACE_GROUP_SIZE, RTXDI_SCREEN_SPACE_GROUP_SIZE, 1] (8x8).
    /// </summary>
    public class NativeRtxdiPTSpatialResamplingPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiPTSpatialResamplingPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.PT.SpatialResampling", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.PtSpatialResampling);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.PtSpatialResampling);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.PtSpatialResamplingCompute);
                ...
                cmd.EndSample(RenderPassMarkers.PtSpatialResamplingCompute);
            }
        }
    }
}
