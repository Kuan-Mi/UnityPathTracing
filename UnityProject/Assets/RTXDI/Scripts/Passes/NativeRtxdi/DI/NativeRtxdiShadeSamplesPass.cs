...
namespace PathTracing
{
    ...
    public class NativeRtxdiShadeSamplesPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiShadeSamplesPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.DiShadeSamples);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.DiShadeSamples);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.DiShadeSamplesCompute);
                ...
                cmd.EndSample(RenderPassMarkers.DiShadeSamplesCompute);
            }
        }
    }
}
