...
namespace PathTracing
{
    ...
    /// Source: <c>UnityProject/Assets/RTXDI/Shaders/LightingPasses/GI/TemporalResampling.hlsl</c>.
    /// numthreads = [RTXDI_SCREEN_SPACE_GROUP_SIZE, RTXDI_SCREEN_SPACE_GROUP_SIZE, 1] (8x8).
    /// </summary>
    public class NativeRtxdiGITemporalResamplingPass : ScriptableRenderPass, IDisposable
    {
        ...
        public NativeRtxdiGITemporalResamplingPass(NativeComputeShader shader, RayTraceShader rs)
        {
        ...
        {
            using var builder = renderGraph.AddUnsafePass<PassData>("NativeRtxdi.GI.TemporalResampling", out var pd);
        ...
        static void ExecutePass(PassData data, UnsafeGraphContext context)
        {
            ...
            {
                cmd.BeginSample(RenderPassMarkers.GiTemporalResampling);
                NativeRtxdiBindings.BindRabCommon(data.Rds, data.Context);
                data.Rs.Dispatch(cmd, data.Rds, w, h);
                cmd.EndSample(RenderPassMarkers.GiTemporalResampling);
            }
            else
            {
                cmd.BeginSample(RenderPassMarkers.GiTemporalResamplingCompute);
                ...
                cmd.EndSample(RenderPassMarkers.GiTemporalResamplingCompute);
            }
        }
    }
}
