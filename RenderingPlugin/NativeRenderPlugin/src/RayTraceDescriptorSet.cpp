...
RayTraceDescriptorSet::RayTraceDescriptorSet(RayTraceShader*          shader,
                                             ID3D12Device*             device,
                                             IUnityLog*                log,
...
RayTraceDescriptorSet::~RayTraceDescriptorSet()
{
    FreeAllocations();
}
...
void RayTraceDescriptorSet::Log(UnityLogType type, const char* msg) const
{
    if (m_log) m_log->Log(type, msg, __FILE__, __LINE__);
    else        printf("[RayTraceDescriptorSet] %s\n", msg);
}

void RayTraceDescriptorSet::Logf(UnityLogType type, const char* fmt, ...) const
{
    char buf[512];
    va_list args;
    va_start(args, fmt);
...
// FreeAllocations
// ---------------------------------------------------------------------------
void RayTraceDescriptorSet::FreeAllocations()
{
...
// ---------------------------------------------------------------------------
bool RayTraceDescriptorSet::AllocateAndWriteDescriptors(const CS_BindingSlot* slots,
                                                         uint32_t slotCount,
                                                         uint32_t slotIdx)
{
...
//   CBVs and ROOT_SRVs are bound as inline root descriptors in Dispatch.
//   SRV_ARRAY / UAV_ARRAY bindings use their own heap (Bindless* objects).
// ---------------------------------------------------------------------------
void RayTraceDescriptorSet::UpdateDescriptors(const CS_BindingSlot* slots,
                                               uint32_t slotCount,
                                               uint32_t slotIdx)
{
    ...
    if (m_srvAllocBase[f] != kInvalidAlloc)
    {
        for (size_t i = 0; i < bindings.size(); ++i)
        {
            ...
            if (b.type == RayTraceBindingType::ROOT_SRV)
            {
                continue; // handled as inline root descriptor in Dispatch
            }
            else if (b.type == RayTraceBindingType::TLAS)
            {
            ...
            else if (b.type == RayTraceBindingType::SRV)
            {
...
// ---------------------------------------------------------------------------
void RayTraceDescriptorSet::RequestResourceStates(const CS_BindingSlot* slots, uint32_t slotCount)
{
    if (!m_d3d12v8) return;
    const auto& bindings = m_shader->GetBindings();
    ...
    for (size_t i = 0; i < bindings.size(); ++i)
    {
        ...
        else if (b.type == RayTraceBindingType::UAV && res)
        {
        ...
        else if (b.type == RayTraceBindingType::SRV_ARRAY &&
        ...
        else if (b.type == RayTraceBindingType::UAV_ARRAY &&
                 slot.objectKind == CS_BindingObjectKind::BindlessUAVTexture && slot.objectPtr)
        {
            // Request UNORDERED_ACCESS on the base texture resource.
            if (res)
                m_d3d12v8->RequestResourceState(res, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        }
    }
    ...
    const CS_BindingSlot* slots, uint32_t slotCount)
{
    ...
    // --- Validate all bindings ---
    {
        ...
        for (size_t i = 0; i < bindings.size(); ++i)
        {
            ...
            switch (b.type)
            {
            case RayTraceBindingType::TLAS:
                kind = "TLAS";
                ok = slot.resourcePtr != 0 ||
                     (slot.objectKind == CS_BindingObjectKind::AccelStruct && slot.objectPtr != 0);
                break;
            case RayTraceBindingType::SRV:
                kind = "SRV";
                ok = slot.resourcePtr != 0;
                break;
            case RayTraceBindingType::UAV:
                kind = "UAV";
                ok = slot.resourcePtr != 0;
                break;
            case RayTraceBindingType::ROOT_SRV:
                kind = "ROOT_SRV";
                ok = slot.resourcePtr != 0 ||
                     (slot.objectKind == CS_BindingObjectKind::AccelStruct && slot.objectPtr != 0);
                break;
            case RayTraceBindingType::CBV:
                kind = "CBV";
                ok = slot.resourcePtr != 0 ||
                     (slot.objectKind == CS_BindingObjectKind::NativeBuffer && slot.objectPtr != 0);
                break;
            case RayTraceBindingType::SRV_ARRAY:
                kind = "SRV_ARRAY";
            ...
            if (!ok)
            {
                Logf(kUnityLogTypeError,
                     "RayTraceDescriptorSet::Dispatch: '%s' binding '%s' (%s, space%u, reg%u) is not set",
    ...
    // Compute per-dispatch slot index (triple-buffer × 2 eyes).
    ...
    {
        Logf(kUnityLogTypeError,
             "RayTraceDescriptorSet::Dispatch [%s]: more than %u dispatches in one frame. Clamping.",
